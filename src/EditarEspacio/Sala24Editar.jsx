...
import './Sala24Editar.css'; // Asegúrate de tener el archivo CSS correspondiente

// Importa la imagen original
import Sala24 from './Sala24.png'; // Ruta a la imagen en tu carpeta assets
...
const CentroMundoX  = () => {
  ...
  const [imagen, setImagen] = useState(Sala24); // Imagen predeterminada es AulaImage
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      querySnapshot.forEach((doc) => {
        ...
        setMaxPersonas(data.maxPersonas || 30);
        setPrecio(data.precio || 90);
        setDescripcion(data.descripcion || 'Descripción no disponible.');
        setImagen(data.imagen || Sala24); // Si no hay imagen, usar la imagen predeterminada
      });
    };
  ...
  const handleEliminar = () => {
    ...
    setMaxPersonas(30);
    setPrecio(90);
    setDescripcion('Permanece abierta las 24 horas del día, y ofrece a los estudiantes la posibilidad de su uso para las actividades académicas, y realizar trabajos en grupo con mayor informalidad.');
    setImagen(Sala24); // Restaurar la imagen original
